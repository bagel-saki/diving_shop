...
- EJSによるコーディング
- WordPress構築可能
- src内の情報は静的ページ（dist）とWordPressテーマ直下のassetsに反映される
...
## 使い方および操作方法
1. ターミナルを開く
2. `cd src`をターミナルに入力（cdとsrcの間には半角スペース要）
3. `npm i`をターミナルへ入力
4. ダウンロードが始まります
...
- 納品時（提出時）は`srcフォルダ`内の`node_modules`を削除すること
- 提出時は`gulp build`コマンドを入力し、フォルダ内の整理を行うこと# diving_shop
